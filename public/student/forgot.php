...
<body >
    ...
    <?php
        ...
        if(array_key_exists('uppword', $_POST))  {
            ...
            if(empty($_POST['email'])  || empty($_POST['password']))  {
              ...
              if($error === '')  {
                $query = mysqli_query($db, "SELECT * FROM student WHERE email = '".$email."'  AND password = '".$password."' ") or die(mysqli_error($db));
                  if(mysqli_num_rows($query) == 1)  {
                    $result = mysqli_fetch_array($query);
                    $_SESSION['student_name'] = $result['student_name'];
                    $_SESSION['student_email'] = $result['email'];
                    $_SESSION['student_password'] = $result['student_password'];
    ...
    <form action=""  method="POST" class="center">
      ...
      <fieldset style="width: 500px;">
         ...
         <input type="submit" name="uppword"  value="Confirm Email and Password"/> 
